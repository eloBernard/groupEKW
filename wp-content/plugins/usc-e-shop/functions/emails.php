...
function usces_send_mail( $para ) {
	...
	$para['reply_name']  = $from_name;
	$para['from']        = $from_address;
	$para['return_path'] = $usces->options['error_mail'];
	...
	$replies = array();
	foreach ( $orders as $order_mail ) {
		if ( is_email( trim( $order_mail ) ) ) {
			$replies[] = $order_mail;
		}
	}

	$para['reply_to'] = isset( $replies[0] ) ? $replies[0] : '';
