...
class ec_db{
	...
	public static function get_pricetier_list( ){
		...
		if ( ! $pricetiers ) {
			$sql = 'SELECT ec_pricetier.product_id, ec_pricetier.price, ec_pricetier.quantity FROM ec_pricetier ORDER BY ec_pricetier.quantity ASC';
			...
			foreach( $cart as $cart_item ){
				if( $cart_item->show_stock_quantity || $cart_item->use_optionitem_quantity_tracking ){
					...
					if( $stock_remaining <= 0 ){
						do_action( 'wpeasycart_cartitem_deleting', $cart_item->tempcart_id );
						self::$mysqli->query( self::$mysqli->prepare( "DELETE FROM ec_tempcart WHERE tempcart_id = %d", $cart_item->tempcart_id ) );
						do_action( 'wpeasycart_cartitem_deleted', $cart_item->tempcart_id );
						...
						do_action( 'wpeasycart_cart_updated' );
						do_action( 'wpeasycart_cartitem_updated', $cart_item->tempcart_id );
					}
				}
			}
	...
	public static function get_cart_data( $session_id ){
		...
		if( !$cart_data && $session_id != '' && $session_id != 'not-set' ){
			...
			$cart_data = self::$mysqli->get_row( self::$mysqli->prepare( "SELECT ec_tempcart_data.*, billing_country.name_cnt AS billing_country_name, shipping_country.name_cnt AS shipping_country_name FROM ec_tempcart_data LEFT JOIN ec_country AS billing_country ON ( billing_country.iso2_cnt = ec_tempcart_data.billing_country ) LEFT JOIN ec_country AS shipping_country ON( shipping_country.iso2_cnt = ec_tempcart_data.shipping_country ) WHERE ec_tempcart_data.session_id = %s AND ec_tempcart_data.session_id != ''", $session_id ) );
			do_action( 'wpeasycart_cart_added', $session_id );
	...
	public static function delete_cartitem( $tempcart_id, $session_id ){
		...
		do_action( 'wpeasycart_cartitem_deleting', $tempcart_id );
			...
			if ( ! $cart_item->allow_backorders ) {
				...
				if ( 1 == $product->use_optionitem_quantity_tracking && $optionitem_quantity && $optionitem_quantity->is_stock_tracking_enabled && $optionitem_quantity->quantity < $cart_item->quantity ) {
					if ( $optionitem_quantity->quantity > 0 ) {
						...
						do_action( 'wpeasycart_cartitem_updated', $cart_item->cartitem_id );
					} else {
						do_action( 'wpeasycart_cartitem_deleting', $cart_item->cartitem_id );
						self::$mysqli->query( self::$mysqli->prepare( "DELETE FROM ec_tempcart WHERE tempcart_id = %d", $cart_item->cartitem_id ) );
						do_action( 'wpeasycart_cartitem_deleted', $cart_item->cartitem_id );
					}
					$stock_valid = false;
	...
	public static function insert_order( $cart, $user, $shipping, $tax, $discount, $order_totals, $payment, $payment_type, $orderstatus_id, $order_notes, $order_gateway ){
		...
		} else if( $cart->shipping_subtotal <= 0 && $order_totals->shipping_total <= 0 ) {
			$shipping_method = "";
		...
		if ( count( $tax->fees ) > 0 ) {
			foreach ( $tax->fees as $tax_fee ) {
				self::$mysqli->query( self::$mysqli->prepare( 'INSERT INTO ec_order_fee( order_id, fee_label, fee_rate, fee_total ) VALUES( %d, %s, %s, %s )', $order_id, $tax_fee->label, $tax_fee->rate, round( $tax_fee->amount, $GLOBALS['currency']->get_decimal_length() ) ) );
			}
		}
	...
	public static function get_download( $download_id ){
		return self::$mysqli->get_row( self::$mysqli->prepare( "SELECT download_id, date_created, UNIX_TIMESTAMP(date_created) AS date_created_timestamp, download_count, order_id, product_id, download_file_name, is_amazon_download, amazon_key FROM ec_download WHERE download_id = %s", $download_id ) );	
	}
	...
	public static function clear_tempcart( $session_id ){
		...
		foreach( $tempcart_ids as $tempcart_id ){
			do_action( 'wpeasycart_cartitem_deleted', $tempcart_id );
		}
	}
	...
	public static function insert_stripe_order( $subscription, $webhook_data, $user ){
		...
		foreach ( $webhook_data->lines->data as $sub_row ) {
			...
			$product = self::get_product( '', $subscription->product_id );
			...
			self::$mysqli->insert(
				...
				array(
					...
					'title'			=> $title,
					'model_number'	=> ( is_object( $product ) ) ? $product->model_number : '',
					'order_date' => date( 'Y-m-d H:i:s' ),
	...
	public static function get_category_values( $product_id ) {
		...
		if ( ! $categories ) {
			$categories = self::$mysqli->get_results( self::$mysqli->prepare( "SELECT DISTINCT ec_category.category_id, ec_category.category_name, ec_category.post_id FROM ec_categoryitem, ec_category WHERE ec_categoryitem.product_id = %d AND ec_category.category_id = ec_categoryitem.category_id AND ec_category.is_active = 1", $product_id ) );
	...
	public static function get_product( $model_number, $product_id = 0 ){
		...
		$sql = "SELECT
				...
				product.subscription_recurring_email,
